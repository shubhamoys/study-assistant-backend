...
// Order/Flashcard/CartItem/Coupon/Library are the only entities ever
// @InjectRepository()'d directly (see OrdersService/CouponsService) —
// checkout's transaction reaches Deck/OrderItem/Payment through the
...
@Module({
  imports: [
    TypeOrmModule.forFeature([Order, Flashcard, CartItem, Coupon, Library]),
  ],
  providers: [
    OrdersService,
    OrdersResolver,
